...
public class ManagerProductServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (command.equals("Search")) {
            ...
            url = "/Laptop/productsearch.jsp?searchstring=" + timkiem;
                ...
                if (error.length() == 0) {
                    switch (command) {
                        case "insert":
                            ...
                            String mota = request.getParameter("mota");
                            long soluong = Long.parseLong(request.getParameter("soluongdaban"));
                            productGet.insertProduct(new Product(new Date().getTime(), maloai, tenSanPham, mathuonghieu,
                                    daidien, mattruoc, matsau, gia, mota, soluong));
                            url = "/admin/manager_product.jsp";
                            break;
                        case "update":
                            ...
                            long soluongx = Long.parseLong(request.getParameter("soluongdaban"));
                            System.out.println("Trước");
                         
                            productGet.updateProduct(new Product(maSanPham, maloaix, tenSanPham, mathuonghieux,
                                    daidienx, mattruocx, matsaux, giax, motax,soluongx));
