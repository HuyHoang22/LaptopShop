...
public class ProductGet {
   ...
   public ArrayList<Product> getListProductByNav(long categoryID, int firstResult, int maxResult) throws SQLException{
        ...
        while (rs.next()) {
            ...
            product.setProductNumber(rs.getLong("soluongdaban"));
            list.add(product);
        }
        return list;
    }
...
//them
public ArrayList<Product> getListSellProduct() throws SQLException {
        Connection connection = DBConnect.getConnecttion();
        String sql = "select  * from product order by soluongdaban desc limit 3 ;";
        PreparedStatement ps = connection.prepareCall(sql);
        ResultSet rs = ps.executeQuery();
        ArrayList<Product> list = new ArrayList<>();
        while (rs.next()) {
...
public boolean insert(Product c) throws SQLException {
    try {
         ...
         ps.setLong(10, c.getProductNumber());
 ...
 public boolean insertProduct(Product c) {
    ...
    public boolean updateProduct(Product c) {
       ...
       // String sql = "UPDATE product SET product_id = ?, category_id = ?, product_name = ?, product_image = ?, product_image_forward = ?, product_image_back = ?, product_price = ?, product_description = ? WHERE product_id = ?";
        String sql = "UPDATE product SET product_id=?, category_id=?, product_name=?, brand_id=?, product_image=?, product_image_forward=?, product_image_back=?, product_price=?, product_description = ?, soluongdaban =? WHERE product_id = ?";
        ...
        try {
         ...
         ps.setLong(11, c.getProductNumber());
            return ps.executeUpdate() == 1;
        } catch (SQLException ex) {
            Logger.getLogger(ProductGet.class.getName()).log(Level.SEVERE, null, ex);
        }
